...
import { Menu, HelpCircle, X, Bell, CheckCircle } from 'lucide-react';
import { Play } from 'lucide-react'; // Added missing icon

// --- MODULE-LEVEL MEMORY CACHE ---
// Survives React destroying and rebuilding the page!
...
export const HomePage = ({ onNavigate, onLoginClick, onMenuToggle, currentUser, userTier }: any) => {
  // Pull magazines from global cache to stop network spam
  ...
  useEffect(() => {
    ...
    const fetchHomeData = async () => {
      // If we have memory, skip the database!
  ...
  return (
    <div className="relative min-h-screen bg-transparent text-white p-6 pb-24">
      ...
      <nav className="relative w-full z-[100] p-4 sm:p-6 flex justify-between items-center bg-black/50 backdrop-blur-md border-b border-zinc-900/50 mb-8 rounded-2xl shadow-lg">
        ...
        <div className="flex items-center justify-center cursor-pointer flex-shrink-0" onClick={() => onNavigate({ action: 'home' })}><img src="https://pub-180171f859f64aa7aadb7001a6b96e65.r2.dev/homepage-graphic-assets/logos/SATURDAY%20AM%20Logo.png" alt="Saturday AM" className="h-12 md:h-16 object-contain drop-shadow-md hover:scale-105 transition-transform" /></div>
        ...
        <div className="flex-1 flex items-center justify-end gap-2 sm:gap-4">
          {currentUser && (
            <div className="hidden sm:flex items-center gap-2 cursor-pointer hover:opacity-80 transition-opacity mr-2" onClick={() => onNavigate({ action: 'profile' })}>
               ...
               <span className="text-[10px] sm:text-xs font-black uppercase tracking-widest text-[#fe9a00]"><span className={userTier === 'premium' ? 'text-purple-400 drop-shadow-[0_0_10px_rgba(168,85,247,0.8)] transition-colors duration-500' : 'text-[#fe9a00]'}>{currentUser.username || 'Reader'}</span></span>
          ...
          <button onClick={() => setShowHelpModal(true)} className="p-2 sm:p-2.5 bg-zinc-900 border border-zinc-700 rounded-full hover:bg-[#fe9a00] hover:border-[#fe9a00] group transition-all"><HelpCircle className="w-4 h-4 sm:w-5 sm:h-5 text-zinc-400 group-hover:text-black transition-colors" /></button>
        </div>
      </nav>
      ...
      {showHelpModal && (
        <div className="fixed inset-0 z-[6000] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 sm:p-6 animate-fade-in" onClick={() => setShowHelpModal(false)}>
          <div className="bg-zinc-950 border border-zinc-800 rounded-2xl w-full max-w-lg shadow-2xl relative flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-zinc-800 bg-zinc-900 rounded-t-2xl"><h2 className="text-xl font-black italic uppercase tracking-wider text-[#fe9a00]">Feature Guide</h2><button onClick={() => setShowHelpModal(false)} className="text-zinc-500 hover:text-white transition-colors"><X className="w-6 h-6" /></button></div>
            <div className="p-6 overflow-y-auto no-scrollbar space-y-8 bg-black rounded-b-2xl"><div><h3 className="text-white font-black uppercase tracking-widest text-sm mb-1">Magazines</h3><p className="text-zinc-400 text-xs font-bold leading-relaxed border-l-2 border-[#fe9a00] pl-3">The latest storylines of our serialized series. Publications range from bi-weekly to monthly.</p></div></div>
          </div>
        </div>
      )}
        ...
        <div className="mb-12 animate-fade-in">
          ...
          <div className="flex gap-4 overflow-x-auto pb-4 px-2 no-scrollbar">
            {recentReads.map((read) => (
              <div key={read.id} onClick={() => onNavigate({ ...read.target, autoOpenChapterId: read.chapter_id, autoOpenPage: read.page_index })} className="relative min-w-[140px] w-[140px] md:min-w-[180px] md:w-[180px] aspect-[2/3] rounded-xl overflow-hidden cursor-pointer group border border-zinc-800 hover:border-[#fe9a00] transition-all shadow-lg flex-shrink-0">

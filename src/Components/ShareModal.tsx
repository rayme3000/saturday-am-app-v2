...
export const ShareModal = ({ isOpen, onClose, series, chapter, currentUser, targetImage }: any) => {
  ...
  // --- BRANDED CANVAS GENERATOR ENGINE (Only used for Panels) ---
  const generateHypeGraphic = (): Promise<Blob> => {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        const scale = Math.min(1080 / img.width, availableHeight / img.height);
        ...
        ctx.fillRect(0, 880, 1080, 8);

        ctx.textAlign = 'center';
        
        ctx.font = '900 italic 56px "Arial", sans-serif';
        ctx.fillStyle = '#ffffff';
        ctx.fillText("I'M READING THIS ON", 540, 960);
        
        ctx.fillStyle = '#fe9a00';
        ctx.fillText("SATURDAY AM!", 540, 1025);
        
        if (series?.title) {
          ctx.font = '800 24px "Arial", sans-serif';
          ctx.fillStyle = '#71717a';
          ctx.fillText(`SERIES: ${series.title.toUpperCase()}`, 540, 1065);
        }
  ...
  const handleMasterShare = async () => {
    if (!targetImage) return;
    setIsGenerating(true);
    
    try {
        const blob = await generateHypeGraphic();
        const file = new File([blob], 'saturday-am-hype.jpg', { type: 'image/jpeg' });
        ...
        if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
            ...
            a.href = url;
            a.download = 'saturday-am-hype.jpg';
            a.click();
            URL.revokeObjectURL(url);
            alert("Custom graphic downloaded! Attach this image to your Threads, Bluesky, or Facebook post.");
        }
    } catch (err) {
        console.error(err);
  ...
  return (
    <div className="fixed inset-0 z-[7000] bg-black/90 backdrop-blur-md flex items-center justify-center p-6 animate-fade-in" onClick={onClose}>
      <div className="bg-zinc-950 border border-zinc-800 p-6 sm:p-8 rounded-2xl w-full max-w-sm flex flex-col items-center text-center shadow-2xl relative" onClick={e => e.stopPropagation()}>
        ...
        {/* --- MODE 1: PANEL SHARE (ONLY SHOWS GRAPHIC BUTTON) --- */}
        {targetImage ? (
          <div className="flex flex-col gap-3 w-full">
            <button 
              onClick={handleMasterShare} 
              disabled={isGenerating}
              className="w-full flex items-center justify-center gap-2 bg-[#fe9a00] text-black font-black uppercase tracking-widest py-4 rounded-xl hover:bg-white transition-all shadow-[0_0_20px_rgba(254,154,0,0.3)] disabled:opacity-50"
            >
              {isGenerating ? (
              ...
              {isGenerating ? 'Building Graphic...' : 'Share This Panel'}
            </button>
          </div>
        ) : (
          /* --- MODE 2: APP/SERIES SHARE (ONLY SHOWS TEXT LINKS & PREVIEW) --- */
          <div className="flex flex-col gap-3 w-full">
            
            {/* Visual preview of what the social media link will look like */}
            <div className="relative w-full h-24 mb-2 rounded-xl overflow-hidden border border-zinc-800">
              <img src="https://pub-180171f859f64aa7aadb7001a6b96e65.r2.dev/homepage-graphic-assets/AM%20App%20load%20screen.jpg" className="w-full h-full object-cover opacity-60" alt="Link Preview" />

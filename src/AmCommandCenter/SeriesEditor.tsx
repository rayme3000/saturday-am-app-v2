...
export const SeriesEditor = ({ Dropzone, ThumbnailCropperModal }: any) => {
  ...
  // DYNAMIC CROPPER STATE
  const [cropFile, setCropFile] = useState<File | null>(null);
  const [cropImageUrl, setCropImageUrl] = useState<string | null>(null);
  const [cropTarget, setCropTarget] = useState<{ charIndex: number, isAlt: boolean, altIndex?: number } | null>(null);
  ...
  const [formData, setFormData] = useState({ 
    ...
    contentRating: 'T', isHidden: false,
        ...
        for (const c of data) {
          if (c.name) {
            ...
            if (!map.has(normalizedName)) {
              ...
              uniqueCreators.push({
                name: c.name.trim(), role: c.role, flagCode: c.flag_code, 
    ...
    const fetchSeriesData = async () => {
      if (targetSeries === 'new') {
        ...
        setFormData({ 
          seriesTitle: '', bannerUrl: '', logoUrl: '', characterUrl: '', 
          bannerPositionDesktop: '50% 50%', bannerPositionMobile: '50% 50%',
          bannerScaleDesktop: 100, bannerScaleMobile: 100, characterAlign: 'center', characterScale: 140, logoScale: 100, logoOffset: 16, 
          cardColor: '#18181b', cardPattern: 'none', synopsis: '', awards: '', hasAwards: false, contentRating: 'T', isHidden: false,    
        ...
        if (selectedSeries) {
          ...
          setFormData({ 
            seriesTitle: selectedSeries.title || '', bannerUrl: selectedSeries.cover_url || '', bannerPositionDesktop: selectedSeries.banner_position_desktop || '50% 50%', bannerPositionMobile: selectedSeries.banner_position_mobile || '50% 50%', bannerScaleDesktop: selectedSeries.banner_scale_desktop || 100, bannerScaleMobile: selectedSeries.banner_scale_mobile || 100, logoUrl: selectedSeries.logo_url || '', characterUrl: selectedSeries.character_url || '', characterAlign: selectedSeries.character_align || 'center', characterScale: selectedSeries.character_scale || 140, logoScale: selectedSeries.logo_scale ?? 100, logoOffset: selectedSeries.logo_offset ?? 16, cardColor: selectedSeries.card_color || '#18181b', cardPattern: selectedSeries.card_pattern || 'none', synopsis: selectedSeries.synopsis || '', awards: selectedSeries.awards || '', hasAwards: selectedSeries.has_awards || false, contentRating: selectedSeries.content_rating || 'T', isHidden: selectedSeries.is_hidden || false,         
            creators: loadedCreators, characters: charData || [] 
          });
        }
      }
    };
  ...
  const handleAddCharacter = () => {
    handleInputChange('characters', [...formData.characters, {
      ...
      headshot_url: '', alt_forms: [], age: '', height: '', location: '', ethnicity: '', first_appearance: '', origin: '', powers: '', weakness: ''
    }]);
  ...
  const handleSaveSeries = async () => {
    ...
    try {
      ...
      const { error: seriesError } = await supabase.from('series').upsert({ 
        slug: activeSlug, title: formData.seriesTitle, synopsis: formData.synopsis, cover_url: formData.bannerUrl, banner_position_desktop: formData.bannerPositionDesktop, banner_position_mobile: formData.bannerPositionMobile, banner_scale_desktop: formData.bannerScaleDesktop, banner_scale_mobile: formData.bannerScaleMobile, logo_url: formData.logoUrl, character_url: formData.characterUrl, character_align: formData.characterAlign, character_scale: formData.characterScale, logo_scale: formData.logoScale, logo_offset: formData.logoOffset, card_color: formData.cardColor, card_pattern: formData.cardPattern, awards: formData.hasAwards ? formData.awards : null, has_awards: formData.hasAwards, content_rating: formData.contentRating, is_hidden: formData.isHidden, creator_name: primaryCreator.name, flag_code: primaryCreator.flagCode, creator_avatar: primaryCreator.avatar, creator_bio: primaryCreator.bio, creator_twitter: primaryCreator.twitter, creator_instagram: primaryCreator.instagram, creator_support_link: primaryCreator.supportLink, updated_at: new Date().toISOString() 
      }, { onConflict: 'slug' });
      ...
      const { error: insertError } = await supabase.from('series_creators').insert(formData.creators.map(c => ({ series_slug: activeSlug, role: c.role || 'Creator', name: c.name, flag_code: c.flagCode, bio: c.bio, avatar_url: c.avatar_url, twitter_url: c.twitter, instagram_url: c.instagram, support_url: c.supportLink, is_visible: c.is_visible })));
      ...
      if (formData.characters.length > 0) {
        const charPayload = formData.characters.map(c => {
           const { id, fullbody_url, alt_form_name, alt_headshot_url, ...rest } = c; 
  ...
  const handleDeleteSeries = async () => {
    ...
    try { 
      ...
      const { data: deletedData, error: seriesErr } = await supabase.from('series').delete().eq('slug', targetSeries).select();
      if (seriesErr) throw seriesErr;
      if (!deletedData || deletedData.length === 0) throw new Error("Supabase RLS Policy blocked the deletion!");
  ...
  const handleCropComplete = async (croppedBlob: Blob) => {
    ...
    try {
      ...
      const filePath = `characters/headshots/${fileName}`;
      const { error } = await supabase.storage.from('assets').upload(filePath, croppedBlob, { contentType: 'image/png' });
      if (error) throw error;
      
      const { data } = supabase.storage.from('assets').getPublicUrl(filePath);
      
      if (cropTarget.isAlt && cropTarget.altIndex !== undefined) {
        handleAltFormChange(cropTarget.charIndex, cropTarget.altIndex, 'url', data.publicUrl);
      } else {
        handleCharacterChange(cropTarget.charIndex, 'headshot_url', data.publicUrl);
      }
    } catch (err: any) {
  ...
  const updatePosition = (newPos: string) => { handleInputChange(previewDevice === 'desktop' ? 'bannerPositionDesktop' : 'bannerPositionMobile', newPos); };
  const updateScale = (newScale: number) => { handleInputChange(previewDevice === 'desktop' ? 'bannerScaleDesktop' : 'bannerScaleMobile', newScale); };
  ...
  return (
    <div className="space-y-6 animate-fade-in-up pb-32 relative">
      ...
      {cropImageUrl && ThumbnailCropperModal && (
        <ThumbnailCropperModal
          ...
          onCropComplete={(newUrl: any) => {
            if (cropTarget) {
              if (cropTarget.isAlt && cropTarget.altIndex !== undefined) {
                handleAltFormChange(cropTarget.charIndex, cropTarget.altIndex, 'url', newUrl);
              } else {
                handleCharacterChange(cropTarget.charIndex, 'headshot_url', newUrl);
              }
            }
            setCropImageUrl(null);
            setCropTarget(null);
      ...
      <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl flex items-end gap-4">
        ...
        {targetSeries !== 'new' && (
          <button onClick={handleDeleteSeries} disabled={isSaving} className={`px-6 py-3 border rounded font-bold uppercase text-[10px] tracking-widest transition-colors ${isSaving ? 'bg-zinc-800 text-zinc-500 border-zinc-700' : 'bg-red-900/20 text-red-500 border-red-900 hover:bg-red-500 hover:text-white'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 border-b border-zinc-800 pb-6">
          ...
          <div>
            ...
            <select value={formData.contentRating} onChange={(e) => handleInputChange('contentRating', e.target.value)} className="w-full bg-black border border-zinc-700 rounded p-3 text-white text-sm focus:border-[#fe9a00] transition-colors">
              <option value="E">E / All Ages</option><option value="Y">Y / Youth (10+)</option><option value="T">T / Teen (13+)</option><option value="OT">OT / Older Teen (16+)</option><option value="M">M / Mature (18+)</option>
            </select>
          </div>
        </div>
             ...
             <div className="mt-6 bg-black p-4 sm:p-6 rounded-xl border border-zinc-800 space-y-6">
               <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-800 pb-4">
                 ...
                 <div className="flex items-center gap-1 bg-zinc-900 p-1 rounded border border-zinc-700 w-max">
                   <button onClick={() => setPreviewDevice('desktop')} type="button" className={`flex items-center gap-1.5 px-3 py-1.5 rounded text-[10px] font-black uppercase tracking-widest transition-colors ${previewDevice === 'desktop' ? 'bg-[#fe9a00] text-black' : 'text-zinc-400 hover:text-white'}`}><Monitor className="w-3 h-3" /> Desktop</button>
                   <button onClick={() => setPreviewDevice('mobile')} type="button" className={`flex items-center gap-1.5 px-3 py-1.5 rounded text-[10px] font-black uppercase tracking-widest transition-colors ${previewDevice === 'mobile' ? 'bg-[#fe9a00] text-black' : 'text-zinc-400 hover:text-white'}`}><Smartphone className="w-3 h-3" /> Mobile</button>
                 </div>
               </div>
               ...
               <div className="flex justify-center bg-zinc-900 rounded-lg p-4 border border-zinc-800 overflow-hidden">
                 <div className={`relative overflow-hidden bg-black border border-zinc-700 shadow-xl transition-all duration-300 ${previewDevice === 'desktop' ? 'w-full aspect-[21/9] sm:aspect-[3/1] rounded-xl' : 'w-[200px] sm:w-[240px] aspect-[4/5] rounded-3xl'}`}>
                   <img src={formData.bannerUrl} alt="Banner Preview" className="w-full h-full object-cover transition-all duration-300" style={{ objectPosition: activePosition, transform: `scale(${activeScale / 100})` }} />
                 ...
                 <div className="w-full md:w-1/2">
                   ...
                   <div className="grid grid-cols-3 gap-2">
                     {POSITION_PRESETS.map((preset) => (
                       <button key={preset.value} type="button" onClick={() => updatePosition(preset.value)} className={`p-2 text-[9px] font-black uppercase tracking-wider rounded transition-colors border ${activePosition === preset.value ? 'bg-[#fe9a00] text-black border-[#fe9a00]' : 'bg-zinc-900 border-zinc-700 text-zinc-400 hover:text-white hover:border-zinc-500'}`}>{preset.label}</button>
                     ))}
                   </div>
                 </div>
        ...
        <div className="grid grid-cols-2 gap-4 border-b border-zinc-800 pb-6">
          <div>
            ...
            <div className="flex items-center gap-3 bg-zinc-900 border border-zinc-700 rounded p-2">
              <input type="color" value={formData.cardColor} onChange={(e) => handleInputChange('cardColor', e.target.value)} className="w-8 h-8 rounded cursor-pointer bg-transparent border-0 p-0" />
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {['#18181b', '#fe9a00', '#dc2626', '#3b82f6', '#16a34a', '#a855f7', '#4f46e5', '#db2777'].map(color => (
                <button key={color} type="button" onClick={() => handleInputChange('cardColor', color)} className="w-5 h-5 rounded-full border border-white/20 hover:scale-110 transition-transform shadow-sm" style={{ backgroundColor: color }} title={color} />
              ))}
            </div>
          </div>
          <div>
            <label className="block text-[10px] font-bold text-zinc-400 uppercase mb-2">Card Pattern</label>
            <select value={formData.cardPattern} onChange={(e) => handleInputChange('cardPattern', e.target.value)} className="w-full bg-zinc-900 border border-zinc-700 rounded p-3 text-white text-xs focus:border-[#fe9a00]">
              <option value="none">Solid Color (None)</option><option value="dots">Micro Dots</option><option value="pinstripes">Subtle Pinstripes</option><option value="mesh">Fine Mesh</option><option value="glow">Top Glow Aura</option><option value="cut">Diagonal Cut</option>
            </select>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-b border-zinc-800 pb-6">
           <div className="flex flex-col gap-4">
             <div className="flex items-center justify-between mb-2">
               ...
               {formData.logoUrl && <button type="button" onClick={() => { if(window.confirm("Remove this Logo?")) handleInputChange('logoUrl', '') }} className="text-[10px] font-bold text-red-500 hover:text-red-400 uppercase tracking-widest flex items-center gap-1"><Trash2 className="w-3 h-3" /> Remove Logo</button>}
             </div>
           ...
           <div>
             <div className="flex justify-between items-center mb-4">
               ...
               {formData.characterUrl && <button type="button" onClick={() => { if(window.confirm("Remove this Render?")) handleInputChange('characterUrl', '') }} className="text-[10px] font-bold text-red-500 hover:text-red-400 uppercase tracking-widest flex items-center gap-1"><Trash2 className="w-3 h-3" /> Remove Render</button>}
             </div>
             <div className="flex gap-4">
               ...
               {formData.characterUrl && (
                 <div className="w-24 sm:w-32 flex-shrink-0 flex flex-col items-center">
                   ...
                   <div className="w-full relative overflow-hidden rounded-md aspect-[2/3] border border-zinc-800 shadow-lg group/charpreview" style={getPatternStyle(formData.cardColor, formData.cardPattern)}>
                     <div className="absolute inset-0 bg-gradient-to-br from-transparent to-black/60 z-0" />
                     <img src={formData.characterUrl} alt="Preview" className={`absolute left-1/2 -translate-x-1/2 max-w-none object-contain z-10 transition-all duration-300 ${formData.characterAlign === 'top' ? 'top-0' : formData.characterAlign === 'center' ? 'top-1/2 -translate-y-1/2' : 'bottom-0'}`} style={{ width: `${formData.characterScale || 140}%`, height: '120%' }} />
                     <div className="absolute inset-x-0 bottom-0 h-[50%] bg-gradient-to-t from-black via-black/80 to-transparent z-20" />
                     {formData.logoUrl && <div className="absolute left-0 right-0 flex justify-center z-30 px-2 transition-all duration-300" style={{ bottom: `${formData.logoOffset ?? 16}px` }}><img src={formData.logoUrl} alt="Logo" className="max-h-24 object-contain drop-shadow-[0_4px_8px_rgba(0,0,0,0.8)]" style={{ width: `${formData.logoScale ?? 100}%` }} /></div>}
                     <div className="absolute inset-0 bg-black/80 z-40 opacity-0 group-hover/charpreview:opacity-100 flex items-center justify-center transition-opacity backdrop-blur-sm cursor-pointer" onClick={() => { if(window.confirm("Remove this Render?")) handleInputChange('characterUrl', '') }}><div className="p-3 bg-red-500/20 text-red-500 rounded-full hover:bg-red-500 hover:text-white transition-colors"><Trash2 className="w-6 h-6" /></div></div>
                   </div>
                 </div>
               )}
             </div>
             ...
             {formData.creators.map((c, i) => {
                ...
                return (
                  <div key={i} className="bg-black p-4 rounded border border-zinc-800 space-y-4 relative">
                    ...
                    <select className="w-full bg-black border border-[#fe9a00]/30 text-[#fe9a00] rounded p-2 text-[10px] font-bold uppercase tracking-widest" onChange={(e) => handleAutofillCreator(i, e.target.value)} value="">
                      <option value="" disabled>-- Quick Fill: Select Existing Creator --</option>
                      {existingCreators.map(ec => <option key={ec.name} value={ec.name} className="text-white bg-zinc-900">{ec.name}</option>)}
                    </select>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      ...
                      <div className="flex gap-2">
                        <select value={selectValue} onChange={(e) => { const nc = [...formData.creators]; nc[i].flagCode = e.target.value === 'OTHER' ? (isCustomFlag ? c.flagCode : '') : e.target.value; handleInputChange('creators', nc); }} className="flex-1 bg-zinc-900 border border-zinc-700 rounded p-3 text-white text-xs focus:border-[#fe9a00]">
                          <option value="">-- Flag --</option>
                          {COUNTRY_CODES.map(country => <option key={country.code} value={country.code}>{country.name}</option>)}
                          <option value="OTHER">Other (Enter Code)</option>
                        </select>
                        {(isCustomFlag || selectValue === 'OTHER') && <input type="text" placeholder="Code" maxLength={2} value={c.flagCode} onChange={(e) => { const nc = [...formData.creators]; nc[i].flagCode = e.target.value.toUpperCase(); handleInputChange('creators', nc); }} className="w-16 text-center bg-zinc-900 border border-zinc-700 rounded p-3 text-white text-xs font-black focus:border-[#fe9a00]" />}
                      </div>
                    </div>
          ...
          <div className="space-y-4">
            {formData.characters.map((char, i) => (
              <div key={i} className="bg-black border border-zinc-800 rounded-xl overflow-hidden">
                <div onClick={() => setExpandedCharIndex(expandedCharIndex === i ? null : i)} className="flex items-center justify-between p-4 bg-zinc-900/50 cursor-pointer hover:bg-zinc-900 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 mr-2">
                      <button onClick={(e) => moveCharacter(i, 'up', e)} className="p-1 hover:bg-zinc-800 rounded text-zinc-500 hover:text-white transition-colors" disabled={i === 0}><ArrowUp className="w-4 h-4" /></button>
                      <button onClick={(e) => moveCharacter(i, 'down', e)} className="p-1 hover:bg-zinc-800 rounded text-zinc-500 hover:text-white transition-colors" disabled={i === formData.characters.length - 1}><ArrowDown className="w-4 h-4" /></button>
                    </div>
                    ...
                    <div className="w-10 h-10 rounded-full bg-zinc-800 border border-zinc-700 overflow-hidden shrink-0 flex items-center justify-center relative">
                      ...
                      {char.headshot_url && <img src={char.headshot_url} className="w-full h-full object-cover relative z-10" onError={(e) => { e.currentTarget.style.display = 'none'; }} />}
                    </div>
                    <div>
                  ...
                  <div className="p-6 border-t border-zinc-800 space-y-6 animate-fade-in">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-zinc-800 pt-6">
                      <div>
                        ...
                        <div className="flex gap-2">
                          <select value={['AM', 'PM', 'Pilot Manga'].includes(char.element) ? char.element : (char.element ? 'Custom' : 'AM')} onChange={(e) => { if (e.target.value === 'Custom') handleCharacterChange(i, 'element', 'Custom Entry'); else handleCharacterChange(i, 'element', e.target.value); }} className={`${['AM', 'PM', 'Pilot Manga'].includes(char.element) || !char.element ? 'w-full' : 'w-1/2'} bg-zinc-900 border border-zinc-700 rounded p-3 text-white text-xs`}>
                            <option value="AM">AM</option><option value="PM">PM</option><option value="Pilot Manga">Pilot Manga</option><option value="Custom">Custom...</option>
                          </select>
                          {(!['AM', 'PM', 'Pilot Manga'].includes(char.element) && char.element) && (
                            <input type="text" placeholder="Custom Affiliation..." value={char.element === 'Custom Entry' ? '' : char.element} onChange={(e) => handleCharacterChange(i, 'element', e.target.value)} className="w-1/2 bg-zinc-900 border border-zinc-700 rounded p-3 text-white text-xs" />
                          )}
                        </div>
                      </div>
                      <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-6 gap-4 border-t border-zinc-800 pt-6">
                      ...
                      <div className="col-span-2 flex gap-2">
                        <select value={(char.first_appearance && !seriesChapters.some(ch => `Chapter ${ch.chapter_number}` === char.first_appearance)) ? 'Custom' : (char.first_appearance || '')} onChange={(e) => { if (e.target.value === 'Custom') handleCharacterChange(i, 'first_appearance', 'Custom Entry'); else handleCharacterChange(i, 'first_appearance', e.target.value); }} className={`${(char.first_appearance && !seriesChapters.some(ch => `Chapter ${ch.chapter_number}` === char.first_appearance)) ? 'w-1/2' : 'w-full'} bg-zinc-900 border border-zinc-700 rounded p-3 text-white text-xs`}>
                          <option value="">-- 1st Appearance --</option>
                          {seriesChapters.map(ch => <option key={ch.id} value={`Chapter ${ch.chapter_number}`}>Ch. {ch.chapter_number}</option>)}
                          <option value="Custom">Custom...</option>
                        </select>
                        {(char.first_appearance && !seriesChapters.some(ch => `Chapter ${ch.chapter_number}` === char.first_appearance)) && (
                          <input type="text" placeholder="Type appearance..." value={char.first_appearance === 'Custom Entry' ? '' : char.first_appearance} onChange={(e) => handleCharacterChange(i, 'first_appearance', e.target.value)} className="w-1/2 bg-zinc-900 border border-zinc-700 rounded p-3 text-white text-xs" />
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col items-center border-t border-zinc-800 pt-6">
                      <label className="block text-[9px] font-bold text-zinc-500 uppercase tracking-widest mb-2 text-center">Main Character Headshot (1:1)</label>
                      <button type="button" onClick={() => { setCropTarget({ charIndex: i, isAlt: false }); document.getElementById('headshot-upload')?.click(); }} className="w-full max-w-[200px] mx-auto mb-3 py-4 border-2 border-dashed border-zinc-700 rounded-xl text-zinc-500 font-black uppercase tracking-widest text-[10px] hover:border-[#fe9a00] hover:text-[#fe9a00] transition-colors bg-black">
                        {char.headshot_url ? 'Replace Main Headshot' : '+ Add Main Headshot'}
                      </button>
                    ...
                    {/* NEW: DYNAMIC ALTERNATE FORMS ARRAY */}
                    <div className="w-full border-t border-zinc-800 pt-6 mt-6">
                      <div className="flex justify-between items-center mb-4">
                         <label className="block text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Alternate Forms</label>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {(char.alt_forms || []).map((alt: any, altIdx: number) => (
                           <div key={altIdx} className="flex flex-col items-center bg-black p-4 rounded-xl border border-zinc-800 relative">
                             ...
                             <input type="text" placeholder="Form Name (e.g. Arodihs)" value={alt.name} onChange={(e) => handleAltFormChange(i, altIdx, 'name', e.target.value)} className="w-full bg-zinc-900 border border-zinc-700 rounded p-2 text-white text-[10px] mb-3 text-center" />
                             <button type="button" onClick={() => { setCropTarget({ charIndex: i, isAlt: true, altIndex: altIdx }); document.getElementById('headshot-upload')?.click(); }} className="w-full mx-auto mb-3 py-3 border-2 border-dashed border-zinc-700 rounded-xl text-zinc-500 font-black uppercase tracking-widest text-[10px] hover:border-[#fe9a00] hover:text-[#fe9a00] transition-colors bg-black flex justify-center">
